...
/*  int lower_freq = (380);
  ...
  //#######################################VPPM TESTING ENDS HERE#######################################

  ofstream txPowerVals("txPowerVals");
  ofstream rxPowerVals("rxPowerVals");
  ofstream distanceVals("rxCoordinates");


  cout<<"This is my first implementation file";

  NS_LOG_UNCOND ("This is my first implementation file");
  Simulator::Run ();
  vlc_channel channel;
  ...
  vlc_DeviceHelper devHelper;

  devHelper.CreateTransmitter("THE_TRANSMITTER");
  devHelper.SetTXSignal("THE_TRANSMITTER",1000,0.5,0.2,4.5,0.5);
  devHelper.SetTrasmitterParameter("THE_TRANSMITTER","Bias",0.5);
  devHelper.SetTrasmitterBoost("THE_TRANSMITTER");
  devHelper.SetTrasmitterParameter("THE_TRANSMITTER","SemiAngle",70);
  devHelper.SetTrasmitterParameter("THE_TRANSMITTER","Azimuth",0);
  devHelper.SetTrasmitterParameter("THE_TRANSMITTER","Elevation",180.0);
  devHelper.SetTrasmitterPosition("THE_TRANSMITTER",0,0,0);
  devHelper.SetTrasmitterParameter("THE_TRANSMITTER","Gain",70);


  devHelper.CreateReceiver("THE_RECEIVER");
  devHelper.SetReceiverParameter("THE_RECEIVER","FilterGain",1);
  devHelper.SetReceiverParameter("THE_RECEIVER","RefractiveIndex",1.5);
  devHelper.SetReceiverParameter("THE_RECEIVER","FOVAngle",70);
  devHelper.SetReceiverParameter("THE_RECEIVER","ConcentrationGain",0);
  devHelper.SetReceiverParameter("THE_RECEIVER","PhotoDetectorArea",1.0e-4);
  devHelper.SetReceiverParameter("THE_RECEIVER","RXGain",0);

  chHelper.AttachTransmitter("THE_CHANNEL","THE_TRANSMITTER",&devHelper);
  chHelper.AttachReceiver("THE_CHANNEL","THE_RECEIVER",&devHelper);
  ...
  //generate TX signal
  std::vector<double> TXPower(GenerateSignal(1000,0.4));

  //Creating of  vlc TX device
  ns3::Ptr<vlc::vlc_NetDeviceTX> tx = CreateObject<vlc::vlc_NetDeviceTX>();
  tx->SetAzmuth(3);
  //setting up parameters of TX device
  //tx->AddTXPower(48.573);
  tx->SetSignal(TXPower);
  tx->SetBias(0.5);
  tx->BoostSignal();
  tx->SetSemiangle(70);
  tx->SetAzmuth(0);
  tx->SetElevation(180.0);
  tx->SetPosition(ns3::Vector(0.0, 0.0, 0.0));
  tx->SetTXGain();


  ns3::Ptr<vlc_NetDeviceRX> rx = CreateObject<vlc_NetDeviceRX> ();
  rx->SetFilterGain(1);
  rx->SetRefractiveIndex(1.5);
  rx->SetFOVAngle(70);
  rx->SetConcentrationGain();
  rx->SetPhotoDectectorArea(1.0e-4);
  rx->SetPosition(Vector(0.0, 0.0,0.0));
  rx->SetRXGain();

 /* //attaching the channel to rx and tx so that link is up
  tx->Attach(&channel);
  rx->Attach(&channel);*/


  channel.Attach(tx);//attaching the transmitter
  channel.Attach(rx);//attaching the receiver



  channel.SetPorpagationLossParametersFromTX(); //setting the loss model from tx
  channel.SetPorpagationLossParametersFromRX(); //setting the loss model from rx

  NS_LOG_UNCOND ("calculating the channel losses at receiver end\n");
  double incr = 0.1;
  double finalZ = 99.0;
  double startZ = 0.1;
  int i = 0;
  while(startZ<finalZ){
	  rx->SetPosition(Vector(0.0, 0.0,startZ));
	  startZ+= incr;
	  NS_LOG_UNCOND ("RX moved to co-ordinates");
	  distanceVals << "RX moved to co-ordinates:  " << startZ <<endl;
	  NS_LOG_UNCOND (startZ);
	  NS_LOG_UNCOND (channel.DoCalcPropagationLossForSignal(i));
	  i++;
  }

  //writing TXPower signals to file
  for(unsigned int txC=0;txC<TXPower.size();txC++){
	  txPowerVals << TXPower.at(txC)<<endl;
  }

  //writing RXPower signal to file
  for(unsigned int rxC=0;rxC< rx->GetRXOpticalPowerSignal().size();rxC++){
	  rxPowerVals << rx->GetRXOpticalPowerSignal().at(rxC)<<endl;
  }
